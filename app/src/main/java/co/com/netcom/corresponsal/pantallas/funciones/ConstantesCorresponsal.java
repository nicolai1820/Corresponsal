package co.com.netcom.corresponsal.pantallas.funciones;

public class ConstantesCorresponsal {

    public static String SHARED_PREFERENCES_INFO_USUARIO ="InfoUsuario";
    public static String SHARED_PREFERENCES_TOKEN="Token";
    public static String SHARED_PREFERENCES_PARAMETRICAS="Parametricas";
    public static String SHARED_PREFERENCES_PARAMETRICAS_BANCO ="ParametricasBanco";
    public static String SHARED_PREFERENCES_LOCALIZACION = "Localizacion";
}
